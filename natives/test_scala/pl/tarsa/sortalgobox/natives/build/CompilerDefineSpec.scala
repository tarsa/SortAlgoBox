...
class CompilerDefineSpec extends CommonUnitSpecBase {
  ...
  it must "serialze empty" in {
    val serialized = CompilerDefine("aName", None).serialize
    serialized mustBe "-DaName"
  }

  it must "serialize some" in {
    val serialized = CompilerDefine("aName", Some("aValue")).serialize
    serialized mustBe "-DaName=aValue"
  }
}
