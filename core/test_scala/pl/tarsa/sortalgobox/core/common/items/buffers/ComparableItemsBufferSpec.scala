...
class ComparableItemsBufferSpec extends CommonUnitSpecBase {
  ...
  new Test[String]("c", "a", "b", "d")("a", "d", "b", "c").register("String")

  class Test[@specialized(Group) Item: Ordering: ClassTag: TypeTag](
      before: Item*)(after: Item*) {
    def register(typeName: String): Unit = {
      typeBehavior[ComparableItemsBuffer[Item]]

      it must s"correctly handle $typeName buffer with base 0" in {
        val array = before.toArray.clone()
        val buffer = ComparableItemsBuffer(5, array, 0)
      ...
      it must s"correctly handle $typeName buffer with base 1" in {
        val array = before.toArray.clone()
        val buffer = ComparableItemsBuffer(5, array, 1)
