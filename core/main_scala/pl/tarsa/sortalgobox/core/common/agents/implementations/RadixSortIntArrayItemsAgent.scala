...
class RadixSortIntArrayItemsAgent(items: Array[Int],
                                  buffer: Array[Int],
                                  base: Int = 0)
    extends RadixSortItemsAgent[Int] {
  override type SelfType = RadixSortIntArrayItemsAgent

  override def withBase(newIndexingBase: Int): SelfType = {
    if (base == newIndexingBase) this
    else new RadixSortIntArrayItemsAgent(items, buffer, newIndexingBase)
  }
